...
for p in projects:
	...
	# Add stacks
	for s in projects[p]:
		insert = 'INSERT INTO stack (title, dimension, resolution, image_base, comment, file_extension, min_zoom_level) '
		insert += 'VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id'
		c.execute( insert, (s.name, s.dim, s.res, s.base_url, s.comment, s.file_ext, s.min_zoom_level) )
