...
def get_stack_info(project_id=None, stack_id=None, user=None):
    ...
    if len(ps_all) > 1:
        return {'error': 'Multiple project - stack associations, but should only be one (PID: ' + str(project_id) + ' SID: ' + str(stack_id) + ').'}
    if len(ps_all) == 0:
        return {'error': 'No project - stack association found, but there should be one (PID: ' + str(project_id) + ' SID: ' + str(stack_id) + ').'}
    ps=ps_all[0]
