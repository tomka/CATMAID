...
(function(CATMAID) {
  ...
  SVGUtil.svgMimeType = 'image/svg+xml';
  ...
  var SVGFactory = function(width, height) {
    ...
    this.svg = document.createElementNS(SVGUtil.namespaces.svg, 'svg');
    this.svg.setAttributeNS(SVGUtil.namespaces.xmlns, "xmlns:xlink",
        SVGUtil.namespaces.xlink);
  ...
  SVGFactory.prototype.addMarker = function(id, element, width, height, refX, refY) {
    var marker = document.createElementNS(SVGUtil.namespaces.svg, 'marker');
  ...
  SVGFactory.prototype.addArrowMarker = function(id, width, height, refX, refY, style) {
    refX = refX || 9;
    refY = refY || 3;
    var arrow = document.createElementNS(SVGUtil.namespaces.svg, 'path');
  ...
  SVGFactory.prototype.createCircle = function(cx, cy, r, style) {
    var circle = document.createElementNS(SVGUtil.namespaces.svg, 'circle');
  ...
  SVGFactory.prototype.createLabeledCircle = function(cx, cy, r, style, label,
    ...
    var text = this.createText(cx + labelOffsetX, cy + labelOffsetY, label,
        labelStyle);
    var group = document.createElementNS(SVGUtil.namespaces.svg, 'g');
  ...
  SVGFactory.prototype.createPolygonPath = function(cx, cy, w, h, points, style) {
    ...
    var path = document.createElementNS(SVGUtil.namespaces.svg, 'path');
  ...
  SVGFactory.prototype.createText = function(x, y, content, style) {
    var text = document.createElementNS(SVGUtil.namespaces.svg, 'text');
  ...
  SVGFactory.prototype.createLine = function(x1, y1, x2, y2, style, options) {
    ...
    switch (options.edgeType) {
      ...
      case 'compound':
      case 'multibezier':
        line = document.createElementNS(SVGUtil.namespaces.svg, 'path');
        var pathComponents = ['M', x1, y1];
        for (var i=0; i<options.controlPoints.length / 2; ++i) {

        }
        pathComponents.push(x2);
        pathComponents.push(x1);
      ...
      case 'segments':
      case 'haystack':
        line = document.createElementNS(SVGUtil.namespaces.svg, 'line');
  ...
  SVGFactory.prototype.save = function(filename) {
    ...
    if (!this.defs) {
      var self = this;
      this.defs = document.createElementNS(SVGUtil.namespaces.svg, 'defs');
    ...
    var blob = new Blob([s], {type: SVGUtil.svgMimeType});
