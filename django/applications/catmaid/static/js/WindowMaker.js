...
var WindowMaker = new function()
{
  ...
  var create3dWebGLWindow = function()
  {
    ...
    // Create a Selection Table and arrange selection table below 3D viewer. To
    // do this, the Selection Table has to be moved out of its split node and
    // moved into a new one that is shared with the 3D viewer.
    var st = WindowMaker.create('selection-table');
    var stSplitNode = st.window.getParent();
    var stSibling = stSplitNode.getSiblingOf(st.window);
    stSplitNode.removeResizeHandle();
    stSplitNode.getParent().replaceChild(stSibling, stSplitNode);

    // Make sibling expand over all available space
    var siblingFrame = stSibling.getFrame();
    ...
    win.getParent().replaceChild(new CMWVSplitNode(win, st.window), win);
    win.getRootNode().redraw();
